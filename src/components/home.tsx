...
const Home: React.FC = () => {
  const items = [
    {
      image: 'https://primefaces.org/cdn/primereact/images/galleria/galleria1.jpg',
      title: 'Título 1',
      content: 'Contenido 1', 
    },
    {
  ...
  const itemTemplate = (item: any) => {
    return (
      <div className="carousel-item">
        <img src={item.image} alt={item.title} className='carousel-image Container' style={{ backgroundColor: 'red'}}/>
        <h3>{item.title}</h3>
        <p>{item.content}</p>
      </div>
    );
